...
import {
  botBid, botChoose, legalPlays,
  trickWinner, sortHand,
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    style.textContent=`
      ...
      *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
      ...
      @keyframes coinpulse{0%,100%{transform:translate(-50%,-50%) scale(1)}50%{transform:translate(-50%,-50%) scale(1.13)}}
  ...
  if(game==='balootOnline')return(
    <div style={{height:'100dvh',overflow:'hidden'}}>
      <BalootOnline profile={profile} onUpdate={setProfile} persist={patch=>persistProfile(profile.uid,patch)} onExit={()=>{setGame(null);setTab('home');}}/>
    </div>
  );
...
function HomeScreen({profile,onGame,onUpdate}){
  ...
  return(
    <div style={{position:'absolute',inset:0,overflowY:'auto',WebkitOverflowScrolling:'touch',paddingBottom:'calc(60px + env(safe-area-inset-bottom,0px) + 12px)',paddingTop:'env(safe-area-inset-top,0px)'}}>
      ...
      <div onClick={()=>onGame('balootOnline')} style={{...card,margin:'0 12px 12px',display:'flex',alignItems:'center',gap:12,padding:'11px 14px',border:'1px solid rgba(46,204,113,.28)',cursor:'pointer'}}>
        <span style={{fontSize:24}}>🌐</span>
        <div style={{flex:1}}>
          <div style={{fontSize:13,fontWeight:900,color:'#2ECC71'}}>بلوت أونلاين</div>
          <div style={{color:'rgba(240,237,229,.55)',fontSize:11}}>٤ لاعبين · فريقان · مع أصدقائك</div>
        </div>
        <span style={{fontSize:18,color:'#2ECC71'}}>‹</span>
      </div>
