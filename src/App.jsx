...
import { getFirestore, doc, getDoc, setDoc, collection, orderBy, limit, getDocs, query, serverTimestamp, onSnapshot, updateDoc } from 'firebase/firestore';
...
function App(){
  ...
  if(loading) return(
    <div style={{...G.page,...G.center,flexDirection:'column',gap:20}}>
      <div style={{fontFamily:"'Scheherazade New',serif",fontSize:42,color:'#F0C040',textShadow:'0 0 24px rgba(240,192,64,.4)'}}>بلوت</div>
      <Spin/>
    </div>
  );
...
function AuthScreen({onDone}){
  ...
  if(step==='setup') return(
    <div style={bg}>
      ...
      <div style={{...G.dim,fontSize:12,letterSpacing:2,marginBottom:26}}>أكمل ملفك</div>
      <div style={box}>
        <div style={{fontSize:16,fontWeight:900,textAlign:'center',marginBottom:18}}>مرحباً {pending?.displayName?.split(' ')[0]||'لاعب'} 👋</div>
        <div style={{marginBottom:12}}>
          <div style={{...G.dim,fontSize:11,fontWeight:700,marginBottom:4}}>مدينتك</div>
          <select style={G.input} value={city} onChange={e=>setCity(e.target.value)}>
            {CITIES.map(c=><option key={c}>{c}</option>)}
          </select>
        </div>
        <div style={{marginBottom:18}}>
          <div style={{...G.dim,fontSize:11,fontWeight:700,marginBottom:8}}>اختر رمزك</div>
          <div style={{display:'flex',flexWrap:'wrap',gap:8,justifyContent:'center'}}>
            {AVATARS.map(a=>(
              <div key={a} onClick={()=>setAv(a)} style={{width:44,height:44,borderRadius:'50%',background:'rgba(16,26,18,.9)',border:`2px solid ${av===a?'#F0C040':'rgba(255,255,255,.08)'}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:22,cursor:'pointer',transform:av===a?'scale(1.1)':'none',transition:'all .2s'}}>{a}</div>
            ))}
          </div>
        </div>
  ...
  return(
    <div style={bg}>
      ...
      <div style={{...G.dim,fontSize:12,letterSpacing:2,marginBottom:30}}>المملكة العربية السعودية</div>
      <div style={box}>
        <div style={{fontSize:17,fontWeight:900,textAlign:'center',marginBottom:8}}>سجّل دخولك</div>
        <div style={{...G.dim,fontSize:12,textAlign:'center',lineHeight:1.6,marginBottom:20}}>سجّل باستخدام Google للحفاظ على تقدمك</div>
        ...
        <div style={{...G.dim,fontSize:10,textAlign:'center',marginTop:12}}>بالمتابعة توافق على شروط الاستخدام</div>
      </div>
    </div>
  );
}
...
function HomeScreen({profile,onGame}){
  ...
  const pb='calc(60px + env(safe-area-inset-bottom,0px) + 12px)';
  const pt='env(safe-area-inset-top,0px)';
  return(
    <div style={{position:'absolute',inset:0,overflowY:'auto',WebkitOverflowScrolling:'touch',paddingBottom:pb,paddingTop:pt}}>
      <div style={{textAlign:'center',padding:'18px 14px 0'}}>
        ...
        <div style={{...G.dim,fontSize:11,letterSpacing:2,marginBottom:8}}>العب · تنافس · افوز</div>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10,padding:'0 14px',marginBottom:12}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{...G.dim,fontSize:11}}>{profile.city} · {profile.wins||0} انتصار</div>
        </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,padding:'0 12px',marginBottom:12}}>
        {modes.map(m=>(
          <div key={m.id} onClick={m.id==='bot'||m.id==='quick'?onGame:undefined}
            ...
            <span style={{...G.dim,fontSize:10,textAlign:'center',lineHeight:1.3}}>{m.sub}</span>
          </div>
        ))}
      </div>
      <div style={{display:'flex',background:'rgba(13,20,16,.75)',border:'1px solid rgba(240,192,64,.1)',borderRadius:14,margin:'0 12px',overflow:'hidden'}}>
        {[['٦.٢م','لاعب'],['٩٨٤','مباراة الآن'],['٤.٨','التقييم']].map(([n,l],i)=>(
          <div key={i} style={{flex:1,textAlign:'center',padding:'10px 4px',borderRight:i<2?'1px solid rgba(255,255,255,.06)':'none'}}>
            ...
            <div style={{...G.dim,fontSize:9,marginTop:1}}>{l}</div>
          </div>
        ))}
      </div>
    </div>
...
function GameScreen({profile,onExit}){
  ...
  const showT=msg=>{tn.current++;setToast({msg,k:tn.current});};
  ...
  return(
    <div style={{width:'100%',height:'100%',background:'radial-gradient(ellipse 90% 70% at 50% 50%,#0F2A14,#07090A)',position:'relative',overflow:'hidden',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'Tajawal,sans-serif',direction:'rtl'}}>
      ...
      <div style={{position:'absolute',top:'calc(env(safe-area-inset-top,0px)+54px)',left:'50%',transform:'translateX(-50%)',display:'flex',flexDirection:'column',alignItems:'center',gap:3,zIndex:10}}>
        ...
        <span style={{...G.dim,fontSize:10,fontWeight:700}}>{players[2].name}</span>
      ...
      <div style={{position:'absolute',left:8,top:'50%',transform:'translateY(-50%)',display:'flex',flexDirection:'column',alignItems:'center',gap:3,zIndex:10}}>
        ...
        <span style={{...G.dim,fontSize:10,fontWeight:700}}>{players[1].name}</span>
      </div>
      ...
      <div style={{position:'absolute',right:8,top:'50%',transform:'translateY(-50%)',display:'flex',flexDirection:'column',alignItems:'center',gap:3,zIndex:10}}>
        ...
        <span style={{...G.dim,fontSize:10,fontWeight:700}}>{players[3].name}</span>
      </div>
      ...
      <div style={{position:'relative',width:190,height:150,zIndex:20}}>
        {trick.map((c,i)=>(
          <div key={i} style={{...G.card(c.s),top:[18,42,10,36][i%4],left:[34,16,54,30][i%4]+'%',transform:`rotate(${[-8,5,-3,10][i%4]}deg)`,zIndex:i+1}}>
      ...
      <div style={{position:'absolute',bottom:'calc(env(safe-area-inset-bottom,0px)+8px)',left:0,right:0,height:106,zIndex:30,display:'flex',justifyContent:'center',alignItems:'flex-end'}}>
        {hand.map((card,i)=>{
          ...
          return(
            <div key={i} onClick={e=>play(e,card,i)}
              style={{...G.card(card.s),left:`calc(50% + ${off}px - 27px)`,transform:`rotate(${rot}deg) translateY(${iS?-26:lft}px) scale(${iS?1.07:1})`,zIndex:iS?90:i+1,border:`1px solid ${iS?'#2ECC71':'rgba(0,0,0,.12)'}`,boxShadow:iS?'0 0 0 2px rgba(46,204,113,.35),0 8px 22px rgba(0,0,0,.7)':'0 8px 22px rgba(0,0,0,.65)',transition:'transform .25s cubic-bezier(.34,1.56,.64,1),box-shadow .2s,border-color .2s'}}>
      ...
      {win&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.88)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:500,padding:20}}>
          <div style={{background:'radial-gradient(ellipse at top,#1A3D20,#0C1410)',border:'1px solid #F0C040',borderRadius:22,padding:'34px 26px',textAlign:'center',boxShadow:'0 0 40px rgba(240,192,64,.25),0 50px 100px rgba(0,0,0,.9)',width:'100%',maxWidth:320,animation:'popIn .5s cubic-bezier(.34,1.56,.64,1)'}}>
            ...
            <div style={{...G.dim,fontSize:13}}>وصلتم إلى ١٥٢ نقطة</div>
            <div style={{display:'flex',justifyContent:'center',gap:28,margin:'18px 0'}}>
              {[['أ',scores.a,'#F0C040'],['ب',scores.b,'rgba(240,237,229,.4)']].map(([t,v,c])=>(
                <div key={t} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:2}}>
                  ...
                  <span style={{...G.dim,fontSize:11}}>الفريق {t}</span>
                </div>
              ))}
            </div>
  ...
  return(
    <div style={{position:'absolute',inset:0,overflowY:'auto',WebkitOverflowScrolling:'touch',paddingBottom:'calc(60px + env(safe-area-inset-bottom,0px) + 12px)',paddingTop:'env(safe-area-inset-top,0px)'}}>
      <div style={{padding:'16px 14px 8px',textAlign:'center'}}>
        ...
        <div style={{...G.dim,fontSize:11,marginTop:4}}>أفضل لاعبي المملكة</div>
      </div>
      ...
      {players.map((p,i)=>(
        <div key={p.id} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 14px',borderBottom:'1px solid rgba(255,255,255,.06)'}}>
          ...
          <div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:700,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{p.name}</div><div style={{...G.dim,fontSize:10}}>{p.city}</div></div>
...
function FriendScreen(){
  ...
  return(
    <div style={{position:'absolute',inset:0,overflowY:'auto',WebkitOverflowScrolling:'touch',paddingBottom:'calc(60px + env(safe-area-inset-bottom,0px) + 12px)',paddingTop:'env(safe-area-inset-top,0px)'}}>
      ...
      <div style={{padding:'0 12px 12px'}}><input style={{background:'rgba(255,255,255,.06)',border:'1px solid rgba(255,255,255,.1)',borderRadius:10,padding:'11px 13px',fontFamily:'Tajawal,sans-serif',fontSize:14,color:'#F0EDE5',width:'100%',outline:'none'}} placeholder="🔍 ابحث عن صديق..."/></div>
      <div style={{height:1,background:'rgba(255,255,255,.07)',margin:'0 14px'}}/>
      {friends.map((f,i)=>(
  ...
  return(
    <div style={{position:'absolute',inset:0,overflowY:'auto',WebkitOverflowScrolling:'touch',paddingBottom:'calc(60px + env(safe-area-inset-bottom,0px) + 12px)',paddingTop:'env(safe-area-inset-top,0px)'}}>
      <div style={{textAlign:'center',padding:'22px 14px 16px',background:'linear-gradient(180deg,rgba(26,61,32,.5),transparent)'}}>
        ...
        <div style={{...G.dim,fontSize:12,marginBottom:14}}>📍 {profile.city}</div>
        <div style={{display:'flex',justifyContent:'center',gap:22}}>
          {[['انتصار',profile.wins||0],['هزيمة',profile.losses||0],['🪙',profile.coins||500]].map(([l,v])=>(
            <div key={l} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:2}}>
              ...
              <span style={{...G.dim,fontSize:10}}>{l}</span>
            </div>
          ))}
        </div>
      </div>
      {editing?(
        <div style={{padding:'0 14px'}}>
          <div style={{marginBottom:10}}><div style={{...G.dim,fontSize:11,fontWeight:700,marginBottom:4}}>الاسم</div><input style={{background:'rgba(255,255,255,.06)',border:'1px solid rgba(255,255,255,.1)',borderRadius:10,padding:'11px 13px',fontFamily:'Tajawal,sans-serif',fontSize:14,color:'#F0EDE5',width:'100%',outline:'none'}} value={name} onChange={e=>setName(e.target.value)} maxLength={20}/></div>
          <div style={{marginBottom:10}}><div style={{...G.dim,fontSize:11,fontWeight:700,marginBottom:4}}>المدينة</div><select style={{background:'rgba(255,255,255,.06)',border:'1px solid rgba(255,255,255,.1)',borderRadius:10,padding:'11px 13px',fontFamily:'Tajawal,sans-serif',fontSize:14,color:'#F0EDE5',width:'100%',outline:'none'}} value={city} onChange={e=>setCity(e.target.value)}>{CITIES.map(c=><option key={c}>{c}</option>)}</select></div>
          <div style={{marginBottom:16}}><div style={{...G.dim,fontSize:11,fontWeight:700,marginBottom:8}}>الرمز</div><div style={{display:'flex',flexWrap:'wrap',gap:8,justifyContent:'center'}}>{AVATARS.map(a=><div key={a} onClick={()=>setAv(a)} style={{width:44,height:44,borderRadius:'50%',background:'rgba(16,26,18,.9)',border:`2px solid ${av===a?'#F0C040':'rgba(255,255,255,.08)'}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:22,cursor:'pointer',transform:av===a?'scale(1.1)':'none',transition:'all .2s'}}>{a}</div>)}</div></div>
        ...
        <div style={{padding:'0 14px',marginTop:8}}>
          {[{i:'✏️',t:'تعديل الملف',s:'الاسم، الرمز، المدينة',a:()=>setEditing(true)},{i:'📊',t:'إحصائياتي',s:'نسبة الفوز وتفاصيل اللعب',a:()=>{}},{i:'🔔',t:'الإشعارات',s:'تحكم في التنبيهات',a:()=>{}},{i:'🚪',t:'تسجيل الخروج',s:'',a:onLogout,red:true}].map((item,i)=>(
            <div key={i} onClick={item.a} style={{display:'flex',alignItems:'center',gap:10,padding:12,background:'rgba(16,26,18,.85)',border:'1px solid rgba(255,255,255,.07)',borderRadius:12,marginBottom:8,cursor:'pointer',touchAction:'manipulation'}}>
              ...
              <div style={{flex:1}}><div style={{fontSize:13,fontWeight:700,color:item.red?'#E74C3C':undefined}}>{item.t}</div>{item.s&&<div style={{...G.dim,fontSize:11}}>{item.s}</div>}</div>
              {!item.red&&<span style={G.dim}>›</span>}
            </div>
          ))}
        </div>
      )}
